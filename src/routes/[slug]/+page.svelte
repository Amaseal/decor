<script lang="ts">
	...
	console.log(data.category.description);
</script>
...
<section class="header">
	<div class="container">
		<div class="flex gap align">
			<div class="images">
				{#each data.category.images as item}
					<img src={item.formats.medium.url} alt="" />
				{/each}
			</div>
			...
			{#if data.products?.length > 0}
				<div class="grid products">
					{#each data.products as product (product.id)}
						<a
							href="/{data.category.slug}/{product.slug}"
							class="product"
							in:fade|local={{ duration: 200, delay: 100 }}
							out:fade|local={{ duration: 100 }}
						>
							<div class="img-container">
								<img src={product.images[0].formats.small.url || product.images.url} alt="" />
							</div>

							{product.title}
						</a>
					{/each}
					{#await data.await.products}
						<p>Loading...</p>
					{:then products}
						{#each products as product}
							<a in:fade href="/{data.category.slug}/{product.slug}" class="product">
								<div class="img-container">
									<img src={product.images[0].formats.small.url || product.images.url} alt="" />
								</div>
